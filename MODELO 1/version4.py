...
- Búsqueda del hueco más amplio en 16 sonares
...
def find_best_gap(readings):
    """
    Busca la ventana de 3 sonares consecutivos con mayor distancia mínima.
    Más robusto que buscar el sonar máximo individual.
    Busca en los 16 sonares para detectar huecos también laterales y traseros.
    """
    best_score  = -1
    best_center = 0
    window = 3
    for i in range(16 - window + 1):
        score = min(readings[i:i + window])
        if score > best_score:
            best_score  = score
            best_center = i + window // 2
    return 1 if best_center <= 7 else -1


def open_turn_speeds(readings, factor):
    """Giro final hacia el hueco más amplio."""
    best_score  = -1
    best_center = 0
    window = 3
    for i in range(16 - window + 1):
...
def main(args=None):
    ...
    while coppelia.is_running():
        ...
        if trap_count >= TRAP_CYCLES:
            ...
            arc_dir          = find_best_gap(readings)
